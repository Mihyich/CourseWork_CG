...
bool LoadModel(const std::string& filePath, std::vector<Vertex>& verteces, std::vector<unsigned int>& indices, bool CCW)
{
...
bool LoadModel(const std::string& filePath, std::vector<vec3>& vertices, std::vector<vec3>& normales, std::vector<unsigned int>& indices, bool CCW)
{
    Assimp::Importer importer;
    const aiScene* scene = importer.ReadFile(filePath, aiProcess_Triangulate | aiProcess_GenNormals | aiProcess_JoinIdenticalVertices);
    aiMesh* mesh = nullptr;
    ...
    if (scene && scene->mNumMeshes)
    {
        ...
        for (size_t t = 0; t < scene->mNumMeshes; ++t)
        {
            ...
            for (unsigned int i = 0; i < mesh->mNumVertices; ++i)
            {
                vec3_set(&position, mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z);
                vec3_set(&normal, mesh->mNormals[i].x, mesh->mNormals[i].y, mesh->mNormals[i].z);
...
bool GenModelMesh(const std::string& filePath, GLuint &VAO, GLuint &VBO, GLuint &EBO, GLsizei &index_count, bool CCW)
{
    ...
    if (!LoadModel(filePath, verteces, indices, CCW))
        return false;
    ...
    glBufferData(GL_ARRAY_BUFFER, verteces.size() * sizeof(Vertex), verteces.data(), GL_STATIC_DRAW);
