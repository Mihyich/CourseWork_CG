...
3. Если есть матрица и треугольник => в жопу ограничивающий объем => трассировка треугольника
    ...
    3.1 Попал: Обновить данные о ближайшем треугольнике (closesT, closestMatrixIndex, closestTriangleIndex);
    Вызвать процедуру подъема по дереву
...
Решение 2: (Очень интересный метод) Пока не поддерживается

Так как, по итогу BVH - это БДДП (почти), если оставшееся дерево, начиная с исследуемого узла является
