...
uniform float lightRadius = 20.0; // Радиус источника света
...
float EstimatePenumbraSize(float fragDepth, float blockerDepth)
{
    ...
    float viewLightFragDepth = (fragDepth * 2.0 - 1.0 - lightProjectionParams.y) / lightProjectionParams.x;
    float viewLightBlockerDepth = (blockerDepth * 2.0 - 1.0 - lightProjectionParams.y) / lightProjectionParams.x;
...
float PerformPCF(vec3 projCoords, float penumbraSize)
{
    ...
    for (int x = -halfSize; x <= halfSize; ++x)
    {
        for (int y = -halfSize; y <= halfSize; ++y)
        {
            ...
            float pcfDepth = texture(shadowMap, projCoords.xy + noiseOffset * texelSize * filterRadius).r;
